...
import { createStripeSdkClient } from "./stripe";
import {
  PaymentProvider,
  Customer,
  Product,
...
export class StripeProvider implements PaymentProvider {
  ...
  getCustomer = (customerId: string): Effect.Effect<Customer, CustomerError> =>
    Effect.gen(function* () {
      const result = yield* Effect.promise(() =>
        this.stripe.getCustomer(customerId)
      );
  ...
  getOrder = (orderId: string): Effect.Effect<Order, EcommerceError> =>
    Effect.gen(function* () {
      ...
      const checkoutSession = yield* Effect.promise(() =>
        this.stripe.getCheckoutSession(orderId)
      );
  ...
  cancelSubscription = (args: {
    ...
    Effect.gen(function* () {
      ...
      const result = yield* Effect.promise(() =>
        this.stripe.cancelSubscriptionAtPeriodEnd(args.subscriptionId)
      );
