...
export const PayPalLive = (options: PayPalOptions) =>
  Layer.scoped(
    ...
    Effect.gen(function* () {
      ...
      const withGate = <A, E>(eff: Effect.Effect<A, E>) =>
        TSemaphore.withPermits(gate, 1)(eff);
          ...
          Effect.scoped(client.execute(req)).pipe(
            ...
            Effect.flatMap(
              HttpClientResponse.matchStatus({
                204: () => Effect.void,
                orElse: (res) =>
                  Effect.fail(new PayPalHttpError({ status: res.status, message: `HTTP ${res.status}` })),
              })
            ),
            Effect.retry(retryPolicy)
          )
      ...
      const captureOrder = ({ orderId, idempotencyKey }: CaptureOrderArgs) =>
        Effect.gen(function* () {
          const req0 = HttpClientRequest.post(
      ...
      const authorizeOrder = ({ orderId, idempotencyKey }: AuthorizeOrderArgs) =>
        Effect.gen(function* () {
          const req0 = HttpClientRequest.post(
