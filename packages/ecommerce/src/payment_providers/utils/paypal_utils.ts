...
export function listAuthorizationIds(orderOrAuthorizeRes: any): string[] {
  const units: any[] = orderOrAuthorizeRes?.purchase_units ?? [];
  const ids: string[] = [];
  for (const u of units) {
    const auths: any[] = u?.payments?.authorizations ?? [];
...
export function listCaptureIds(orderOrCaptureRes: any): string[] {
  const units: any[] = orderOrCaptureRes?.purchase_units ?? [];
  const ids: string[] = [];
  for (const u of units) {
    const captures: any[] = u?.payments?.captures ?? [];
